...
# Rita ut vårt spel, en "pixel" åt gången
def draw(surface: pygame.Surface):
    for y in range(GRID_HEIGHT):
        for x in range(GRID_WIDTH):
            color = LIGHT_PINK if (x+y)%2 else DARK_PINK
            r = pygame.Rect((x*GRID_PIXEL_SIZE, y*GRID_PIXEL_SIZE), (GRID_PIXEL_SIZE, GRID_PIXEL_SIZE))
    ...
    while (True):
        ...
        if snake.is_alive:
            ...
            if snake.positions[0] == food.position:
                snake.score += 1
                food.position = get_random_position()
            snake.draw(surface)
            food.draw(surface)
            screen.blit(surface, (0,0))
            text = myfont.render("Score {0}".format(snake.score), True, BLACK)
            screen.blit(text, (5,10))
            pygame.display.update()
        else:
            screen.blit(surface, (0,0))
            text = myfont.render(f"YOU DIED! Score: {snake.score} Press enter to play again.", True, BLACK)
            screen.blit(text, (120, MIDDLE_COORDINATES[1]))
            pygame.display.update()
